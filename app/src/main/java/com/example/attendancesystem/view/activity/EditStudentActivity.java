...
public class EditStudentActivity extends AppCompatActivity2 {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        addStudentToolbar.setTitle("Edit Student");
        ...
        if (bundle != null) {
             studentStart = (Student) bundle.getSerializable("student");
            intentDept= studentStart.getDepartment();
            intentBatch=studentStart.getBatch();
            intentShift=studentStart.getShift();
            addStudentID.setText(studentStart.getId());
            addStudentName.setText(studentStart.getName());
            addStudentEmail.setText(studentStart.getEmail());
            addStudentPhone.setText(studentStart.getPhone());
            SelectedYear = studentStart.getYear();
            SelectedSemister = studentStart.getSemester();


            Picasso.get().load(studentStart.getPathImage())
                    .error(R.drawable.ic_account)
                    .placeholder(R.drawable.ic_account)
                    .into(imageAccount);
            imageUrl = studentStart.getPathImage();
        }
        ...
        coureRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if(dataSnapshot.exists()){
                    ...
                    CheckableSpinnerAdapter cadapter = new CheckableSpinnerAdapter<>(EditStudentActivity.this,headerText,course_spinner_items,course_spinner_code,course_selected_items,selected_course);
        ...
        int position = getPositionByValue(year, SelectedYear);
        if (position != -1) {
            studentYearSp.setSelection(position);
        }

        int position2 = getPositionByValue(semester, SelectedSemister);
        if (position2 != -1) {
            studentSemesterSp.setSelection(position2);
        }
...
//               addStudent();
    ...
    private int getPositionByValue(String[] year, String selectedYear) {
        for (int i = 0; i < year.length; i++) {
            if (year[i].equals(selectedYear)) {
                return i;
            }
        }
    ...
    private void EditStudent(){
        ...
        String phone=addStudentPhone.getText().toString();
            ...
            final Student studentup=new Student(name,ID,SelectedYear,SelectedSemister,intentDept,intentBatch,"",email,phone,"",stringBuildert.toString(),stringBuilderc.toString(),intentShift,"1234",imageUrl);
                    ...
                    if(dataSnapshot.exists()) {
                        for (DataSnapshot dataSnapshot1 : dataSnapshot.getChildren()) {
                            if (dataSnapshot1.hasChildren()) {
                                ...
                                if(studentFB.getId().toString().trim().equals(studentup.getId().toString().trim()) ){
                                    studentRef.child(dataSnapshot1.getKey())
                                            .setValue(studentup)
